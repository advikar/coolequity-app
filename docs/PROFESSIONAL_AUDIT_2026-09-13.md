...
> | Chooser + intro friction, three-column squeeze, mobile legend | **Fixed** | Chooser cards and the in-map city switch open the map directly (`?go=1`); the start screen remains for direct links and the home button. The city note (e.g. "heat shown, not scored") now also sits under the panel lead. "Color the map by", "Places to cool off" and "Export & share" start folded (a viewer's own toggle is remembered), so priorities and the ranked list fit without scrolling on a laptop. Legend starts collapsed under 700px |
