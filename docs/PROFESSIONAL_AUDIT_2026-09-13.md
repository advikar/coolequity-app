...
> | Mixed canopy sources / reliability flag at point of use | **Fixed (Contra Costa builds); Bakersfield pooled** | Detail panel tags tree cover with source, year and assessed share. Cells whose aerial assessment covered under 10% (`CANOPY_MIN_COVERAGE`) now score on the satellite stand-in: 121 Contra Costa, 63 Bakersfield, 20 San Ramon cells; top-25 unchanged in every city. Legacy height-model cells and full/partial aerial cells are still pooled in one input Sixth pass: ranked rows now label the source ("trees", "trees · model", "trees · partial", "greenness") with the explanation on hover, and a "Full aerial tree data" filter drops every non-aerial or partly assessed area; a "Stable top tier" filter keeps only areas in the top tier in ≥80% of stability re-draws. Sources are still pooled in one normalised input Eleventh pass: published accuracy for each source is now recorded (Meta/WRI urban validation 86% overall, canopy share within 3 points per city; NAIP deep-learning canopy misses crowns under 1.5 m; NLCD 10 points low in cities) and an independent lidar reference was found for every Contra Costa build: CDFW's 2025 Alameda–Contra Costa vegetation map (ds3206) carries lidar canopy cover per polygon. **Run the same evening** (`pipeline/02f_canopy_validate.py`): aerial and height model both track the lidar (Spearman 0.97 in the county) but on lines 2.4 points apart, so 05 now rescales height-model cells onto the aerial line (county build, 1,330 cells, guarded by fit n ≥ 50 and Spearman ≥ 0.8) and replaces the greenness stand-in with lidar canopy wherever the map covers the cell (stand-in cells left: county 5, West County 1, Pittsburg 0, San Ramon 0; Pittsburg's stand-in had Spearman −0.05 against lidar). Top-25 unchanged in three builds, 24/25 in West County. Results per build in `reports/canopy_validation_<slug>.json`; sources labelled in the app and exports. Bakersfield is outside the map and keeps the pooled input |
> | Uncertainty / rank stability bands | **Fixed (first version)** | New `05b_stability.py`: 300 re-draws of ACS block-group population and 65+ counts and LACE tract A/C inside their published 90% margins (folded onto cells by overlap share, so cells from one block group move together), plus ±10-point jitter of each non-zero recommended weight and of the population slope. Each ranked cell carries `rank_lo`/`rank_hi` (5th–95th percentile) and `rank_top_share`; the detail panel shows "Likely rank a–b · in the top N in x% of re-draws", exports carry the three columns, every guide has a Rank stability paragraph with that city's numbers (median band: county 216 ranks of 2,697; West CC 127 of 982; Pittsburg 123 of 570; Bakersfield 444 of 3,767; San Ramon 51 of 419). Held fixed: heat, canopy, within-block-group allocation |
...
> | Planting outputs: low/base/high cost, survival | **Partly fixed (survival, cost framing)** | New editable survival share (default 70%) beside the cost: canopy gain and cooling are credited only to surviving trees, cost to every planted tree, shown as "≈" and labelled planting-only. Exported (`scenario_trees_planted`/`scenario_trees_surviving`, `survival_share_pct`), restored from scenario files (older files reproduce exactly at 100%), reset with the map, documented in every guide with the USFS Sacramento 22-year figure (42.4%). Eleventh pass: Low $500 / Base $2,000 / High $3,500 presets beside the cost field, each sourced (Bakersfield's own estimate; Concord's $1M-for-500 grant; San Francisco's 3500 Trees at $12M, with Los Angeles's $4,351 as the upper mark); guides rewritten with the sources; evidence in `docs/EVIDENCE_COST_CANOPY_2026-09-13.md`. Default stays at the low band so older files reproduce. Still not done: a Contra Costa or Kern contract price (none published online), lifecycle cost, species/crown ranges, maturity horizon |
> | Chooser + intro friction, three-column squeeze, mobile legend | **Fixed** | Chooser cards and the in-map city switch open the map directly (`?go=1`); the start screen remains for direct links and the home button. The city note (e.g. "heat shown, not scored") now also sits under the panel lead. "Color the map by", "Places to cool off" and "Export & share" start folded (a viewer's own toggle is remembered), so priorities and the ranked list fit without scrolling on a laptop. Legend starts collapsed under 700px. Seventh pass: on phones a List / Both / Map switch in the panel header gives either the full screen (remembered per browser) |
> | Generated cell names look too precise | **Fixed** | Detail subtitle shows the stable area id; name tooltip says it is generated. Seventh pass: every cell carries the city or census-designated community containing its centre (`04d_jurisdiction.py`, Census TIGERweb places); shown in the detail line and hover, exported as `city_or_community`, and a "Show" menu limits the list to one city and fades the rest of the map (the audit's "filter jurisdiction") |
> | Decision workflow: field verification, status, briefing packet | **Fixed (first version)** | Field check box in every ranked area's detail panel (status + note, saved per city in the browser, never uploaded); status tag in the ranked list; `field_check_*` columns in area and shortlist exports; shown under each shortlisted area in the printed briefing. Ninth pass: shortlist "Compare" shows up to six starred areas side by side (likely rank, city, residents, 65+, heat, tree cover with source, A/C, walk, scenario trees and cost, field check), the greatest need on each row marked; the briefing's top-25 table gains City and Likely rank, and each shortlisted area's heading carries band, city and id |
> | Test the deployed build, not only the checkout | **Fixed** | `smoke` job in the Pages workflow: after each deploy, every city's map, guide, cooling page and data must answer 200 on the live site (retried for up to 8 minutes for the Pages cache) and the chooser must list every city |
> | West CC and Pittsburg guides were rewritten county copies | **Resolved (deliberately not a full rewrite)** | Method topics are the same method in every city and stay identical on purpose. Each of the two guides now opens with a fresh "About this study area" topic (what it covers, what the data shows, how it differs from the county map, where the data is weakest, with that build's own numbers) and a rewritten "What this build found" topic; the last county-only phrases and the source-code link were fixed |
...
> | Area weighting places residents on water (found while building Pittsburg) | **Fixed** | `03_census.py` area-weighting fallback gives no residents to cells whose dominant WorldCover class is open water or wetland. Pittsburg: 3,796 residents off 153 marsh/bay cells. Bakersfield (the other build without a building mask): 351 residents off 21 cells, ranked areas 3,788 to 3,767, top 25 unchanged, top 100 99/100 |
> | Fifth build: Pittsburg & Bay Point | **Done** | 570 ranked areas, 89,581 residents. Heat measured on this build's own cells and NOT scored (position explains 28% of LST among warm residential cells; LST vs canopy r = +0.25), so the county's weights apply. Caveats in its guide: OSM building mask 8% (area weighting), only 8 OSM cool places, no HOLC map |
>
