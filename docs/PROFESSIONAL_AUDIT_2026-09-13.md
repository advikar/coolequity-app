...
> | Decision workflow: field verification, status, briefing packet | **Fixed (first version)** | Field check box in every ranked area's detail panel (status + note, saved per city in the browser, never uploaded); status tag in the ranked list; `field_check_*` columns in area and shortlist exports; shown under each shortlisted area in the printed briefing. Side-by-side comparison of 3–5 areas is still the shortlist + briefing, not a dedicated compare view |
