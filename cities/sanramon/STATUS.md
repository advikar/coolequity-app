> **History.** This is the `STATUS.md` of the `san-ramon` branch of [advikar/coolequity](https://github.com/advikar/coolequity/tree/san-ramon), carried over unchanged when the cities were combined into this repository (September 12, 2026). Paths, branch names, run and deploy instructions in it refer to that repository; the top-level README.md is current.
